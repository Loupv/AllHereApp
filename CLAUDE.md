...
app/                          expo-router tree
  _layout.tsx                 root Stack, fonts, splash, Player overlay
  ...
  silent-mind-tree.tsx        SM journey screen — paged vertical "tree"
...
- `src/player/progressStore.ts` — `listened` map (trackId → true) for
  phase transitions on Start. Persisted via `kv` (hand-rolled init +
  write, not zustand `persist` — see the note in the file).
- `src/player/notificationStore.ts` — unread badges on Media tab.
- `src/content/kv.ts` — key/value shim: `localStorage` on web,
  `AsyncStorage` on native (disk-backed, survives relaunches). Also
  surfaced as a zustand `StateStorage` adapter.
...
- **Big leaf files**: `app/(tabs)/index.tsx` (~560 lines) and
  `src/components/Player.tsx` (~2300 lines) deliberately stay monolithic
...
- No AI attribution in commit messages — no `Co-Authored-By` trailer,
  no "generated by" credit, nothing. Subject + optional body only.
