...
- **Silent Mind (SM)** — guided, untimed practices. Brand accent:
  magenta `#9E3694`.
- **QM Format (QM)** — Quantified Meditation. Same practices, but
...
other program. This is what the "Go to QM Format · Part 1" and
...
| QM Format | `app/(tabs)/qm.tsx` | QM program overview → Part 1/2/3 list. |
...
- **Uppercase means state.** Overline/uppercase labels are reserved for
  true state labels (`ROUND 1/3`, `GATEWAY COMPLETED`, `QM FORMAT ·`
